...
def main():
    ...
    if st.button("Predict"):
        ...
        if input_text:
            if output == 1:
                #st.header("Spam")
                st.markdown(f'<h1 style="color:	#A9A9A9;font-size:20px;">{"Spam"}</h1>', unsafe_allow_html=True) 
            else:
                #st.header("Not Spam")
                st.markdown(f'<h1 style="color:	#A9A9A9;font-size:20px;">{"Not Spam"}</h1>', unsafe_allow_html=True) 
        else:
